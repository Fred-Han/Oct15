...
namespace Lean.Touch
{
	...
	{
		private static List<LeanFinger> allFingers = new List<LeanFinger>();   // new List<LeanFinger> 

		private static GUIStyle fadingLabel;         

		[MenuItem("GameObject/Lean/Touch", false, 1)]   // "GameObject/Lean/Touch", 
		...
		{
			if (LeanTouch.Instances.Count > 1) // 如果实例大于1 
			{
		...
		private void DrawSettings(LeanTouch touch)  // private void DrawSettings 
		{
			...
			DrawDefault("RecordFingers");    // 记录手指  
			...
			{
				EditorGUI.indentLevel++;
					DrawDefault("PinchTwistKey");  // PinchTwistKey 
	...
	public partial class LeanTouch : MonoBehaviour
	{
		...
		private void PollFingers()
		{
			...
			else if (AnyMouseButtonSet == true)
			{
				...
				var mousePosition = (Vector2)Input.mousePosition;  // HANFENGHANFENG 


			//	var finger1 = (Vector2)Input.GetTouch(0).position;

			//	var finger1 = new Vector2(1, 2);
			//	var finger2 = (Vector2)Input.GetTouch(1).position;
				...
				if (screen.Contains(mousePosition) == true)
				{
					...
					if (SimulateMultiFingers == true)
					{
					    ...
					    if (Input.GetKey(PinchTwistKey) == true)
						{
							...
							// AddFinger(1, finger1, 1.0f);    // hanfeng 
						}
						// Simulate multi drag?
